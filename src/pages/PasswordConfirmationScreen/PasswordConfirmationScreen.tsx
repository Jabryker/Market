...
const PasswordConfirmationScreen: FC = () => {
  return (
    <div className={styles.PassConfWrapper}>
      ...
      <p className={styles.PassConfSubTitle}>Письмо с подтверждением было отправлено на
                example@gamil.com. Пожалуйста, перейдите по ссылке
                внутри письма, чтобы начать использовать Marketplace.</p>
      <div className={styles.PassConfComWrapper}>
        <button>
          <span>Отправить письмо ещё раз</span>
        </button>
        <HelpText linkTo="/help" linkText="Свяжитесь с нами">Есть жалобы? </HelpText>
      </div>
    </div>
  );
};
